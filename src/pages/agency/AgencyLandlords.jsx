...
function AgencyLandlords() {
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      setLookupError(err.response?.data?.error || 'Could not find that user.')
    } finally {
      setIsLooking(false)
    }
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12">
      ...
      {showForm && (
        ...
        >
          ...
          <p className="text-sm text-charcoal/60">
            The landlord must already have a NEST account with the Landlord role.
          </p>

          <div>
            <label className="block text-sm font-medium text-charcoal mb-1.5">Landlord email</label>
            <div className="flex gap-2">
              <input
                ...
                required
                className="flex-1 px-3 py-2.5 rounded-lg border border-clay/30 bg-white text-charcoal placeholder:text-charcoal/30 focus:outline-none focus:ring-2 focus:ring-clay transition text-sm"
              ...
              <button
                ...
                disabled={isLooking}
                className="bg-sienna text-sand px-4 py-2 rounded-lg text-sm font-medium hover:bg-clay transition disabled:opacity-60"
              >
                {isLooking ? 'Looking...' : 'Look up'}
              </button>
            </div>
            {lookupError && <p className="text-sm text-brick mt-1">{lookupError}</p>}
            {foundLandlord && (
              <p className="text-sm text-olive mt-1">
                Found: {foundLandlord.name} ({foundLandlord.email})
              </p>
            )}
          </div>
      ...
      <div className="space-y-3">
        {landlords.map((l) => (
          <div key={l.id} className="bg-white border border-clay/15 rounded-xl p-5">
